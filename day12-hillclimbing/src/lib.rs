...
pub fn breadth_first_search(grid: &mut Grid, start: Point, to_find: char) -> (usize, Point) {
    ...
    while let Some(point) = queue.pop_front() {
        if grid[point.0][point.1].0 == to_find {
            return (count_path_len(parents, point), point);
        }
...
pub fn breadth_first_descend(grid: &mut Grid, start: Point, to_find: char) -> (usize, Point) {
    let mut queue: VecDeque<Point> = VecDeque::new();
    let mut parents: HashMap<Point, Point> = HashMap::new();
